...
describe("DLPRoot", () => {
  ...
  describe("MigrateStakeToVanaPool", () => {
    const initialRewardsTreasuryBalance = parseEther(1000);
    ...
    const VANA_POOL_ROLE = ethers.keccak256(
      ethers.toUtf8Bytes("VANA_POOL_ROLE"),
    );
    ...
    beforeEach(async () => {
      ...
      await rootStaking.connect(owner).updateRewardClaimDelay(0);
      await rootStaking.connect(owner).updateVanaPoolStaking(vanaPoolStaking);
      ...
      await rootStaking.connect(owner).updateStakeLastBlockNumber(999999999);
      ...
      await rootStaking.connect(owner).updateStakeWithdrawalDelay(0);
    });

    it("should migrateRewardToVanaPool for an open stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      await rootStaking
        .connect(user1)
        .migrateRewardToVanaPool(2, 1)
        .should.emit(rootStaking, "RewardMigratedToVanaPool")
        .withArgs(2, stake2ExpectedClaimableAmount, 1);

      (await ethers.provider.getBalance(user1)).should.lt(user1InitialBalance);

      (await vanaPoolStaking.stakerEntities(user1, 1)).shares.should.eq(
        stake2ExpectedClaimableAmount,
      );

      (await rootStaking.calculateStakeClaimableAmount.staticCall(2)).should.eq(
        0,
      );
    });

    it("should migrateRewardToVanaPool for an open stake multiple times", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      let stake1 = await rootStaking.stakes(1);
      let stake2 = await rootStaking.stakes(2);
      ...
      (await rootEpoch.epochs(2)).dlpIds.should.deep.eq([1]);
      ...
      const stake2ClaimableAmount2 =
        await rootStaking.calculateStakeClaimableAmount.staticCall(2);

      stake2ClaimableAmount2.should.eq(stake2ExpectedClaimableAmount);
      stake2ClaimableAmount2.should.eq(parseEther(0.8));
      ...
      (await vanaPoolStaking.stakerEntities(user1, 1)).shares.should.eq(
        stake2ExpectedClaimableAmount * 2n,
      );
    ...
    it("should migrateRewardToVanaPool for a closed stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
    ...
    it("should migrateStakeToVanaPool an active stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      await rootStaking
        .connect(user1)
        .migrateStakeToVanaPool(2, 1)
        .should.emit(rootStaking, "StakeMigratedToVanaPool")
        .withArgs(2, stake2.amount, 1);
      ...
      const stake2After = await rootStaking.stakes(2);
      stake2After.endBlock.should.eq(await ethers.provider.getBlockNumber());
      stake2After.withdrawn.should.eq(true);

      (await ethers.provider.getBalance(user1)).should.lt(user1InitialBalance);
    });

    it("should migrateStakeToVanaPool a closed stake", async function () {
      await rootStaking.connect(owner).updateStakeWithdrawalDelay(100);
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      const stake2After = await rootStaking.stakes(2);
      stake2After.endBlock.should.eq(
    ...
    it("should reject migrateStakeToVanaPool an withdrawn stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
    ...
    it("should reject migrateStakeToVanaPool an migrated stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      await rootStaking.connect(user1).migrateStakeToVanaPool(2, 1).should.be
        .fulfilled;
    ...
    it("should migrateStakeAndRewardToVanaPool an active stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      await rootStaking
        .connect(user1)
        .migrateStakeAndRewardToVanaPool(2, 1)
        .should.emit(rootStaking, "StakeMigratedToVanaPool")
        .withArgs(2, stake2.amount, 1)
        .and.emit(rootStaking, "RewardMigratedToVanaPool")
        .withArgs(2, stake2ExpectedClaimableAmount, 1);

      (await vanaPoolStaking.stakerEntities(user1, 1)).shares.should.eq(
        stake2.amount + stake2ExpectedClaimableAmount,
      );
    ...
    it("should migrateStakeAndRewardToVanaPool a closed and withdrawn stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      await rootStaking
        ...
        .should.emit(rootStaking, "RewardMigratedToVanaPool")
        .withArgs(2, stake2ExpectedClaimableAmount, 1)
        .and.not.emit(rootStaking, "StakeMigratedToVanaPool");
    ...
    it("should migrateStakeAndRewardToVanaPool a closed and not withdrawn stake", async function () {
      await rootStaking.connect(owner).updateStakeWithdrawalDelay(100);
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
    ...
    it("should migrateStakeAndRewardToVanaPool an withdrawn stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
    ...
    it("should reject migrateStakeAndRewardToVanaPool when noth9ng to calim", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
    ...
    it("should migrateStakeAndRewardToVanaPool for an migrated stake", async function () {
      await advanceToEpochN(1);
      const stakerPercentage = parseEther(60);
      await rootCore
        .connect(dlp1Owner)
      ...
      (await vanaPoolStaking.stakerEntities(user1, 1)).shares.should.eq(
        stake2.amount + stake2ExpectedClaimableAmount * 2n,
      );
      ...
      (await rootEpoch.epochs(3)).dlpIds.should.deep.eq([1]);
      ...
      const stake2ClaimableAmount3 =
        await rootStaking.calculateStakeClaimableAmount.staticCall(2);

      stake2ClaimableAmount3.should.eq(0);
