...
import { Tick, TickMath, Pool, Position, maxLiquidityForAmounts, nearestUsableTick, computePoolAddress } from '@uniswap/v3-sdk';
...
const getSqrtPriceLimitX96 = async (pool: any, zeroForOne: boolean, slippageTolerance: bigint): Promise<bigint> => {
    const slot0 = await pool.slot0();
    const sqrtPriceX96: bigint = BigInt(slot0.sqrtPriceX96);
    const slippageFactor = zeroForOne ? ONE_HUNDRED_PERCENT - slippageTolerance : ONE_HUNDRED_PERCENT + slippageTolerance;
    const slippageFactorScaled = slippageFactor * Q96 * Q96 / ONE_HUNDRED_PERCENT;
...
describe("UniswapV3", () => {
    ...
    const deploy = async (testChainId: number = 1480) => {
        ...
        dataDexRouter = await ethers.getContractAt(
            SwapRouter.abi,
            routerAddress,
        );

        dataDexQuoterV2 = await ethers.getContractAt(
            QuoterV2.abi,
            quoterV2Address,
        );
        ...
        tickLens = await ethers.getContractAt(
            TickLens.abi,
            tickLensAddress,
        );
        ...
        WVANA = await ethers.getContractAt(
            IWVANA.abi,
            WVANAAddress,
        );
        ...
        (await factory.getPool(WVANAAddress, TokenAddress, FeeAmount.MEDIUM)).should.equal(poolAddress);
    ...
    describe("SwapHelper", () => {
        ...
        it("should quoteExactInputSingle", async () => {
            ...
            (await swapHelper.getPool(WVANAAddress, ERC20Token, FeeAmount.MEDIUM)).should.equal(poolAddress);

            const liquidity = await pool.liquidity();
            const sqrtPriceX96 = await pool.slot0().then((slot: any) => slot.sqrtPriceX96);
        ...
        it("should exactInputSingle without initializedTicksCrossed", async () => {
            ...
            tickBefore.should.be.eq(TickMath.getTickAtSqrtRatio(JSBI.BigInt(sqrtPriceX96Before.toString())));
            ...
            tickAfter.should.be.eq(TickMath.getTickAtSqrtRatio(JSBI.BigInt(sqrtPriceX96After.toString())));
        ...
        it("should simulateSwap when zeroForOne, exactIn", async () => {
            await fc.assert(
                fc.asyncProperty(fc.integer({ min: 1, max: 10_000 }), async (amount) => {
            ...
            await fc.assert(
                fc.asyncProperty(fc.integer({ min: 1, max: 10_000 }), async (amount) => {
                    ...
                    try {
                        ...
                        const sqrtPriceLimitX96 = await getSqrtPriceLimitX96(pool, true, SLIPPAGE_TOLERANCE);
                        ...
                        let quoteSlippage = await swapHelper.quoteSlippageExactInputSingle.staticCall({
                            tokenIn: PoolToken0,
                            tokenOut: PoolToken1,
                            fee: FeeAmount.MEDIUM,
                            amountIn: amountIn,
                        ...
                        if (simulateResult.amountToPay < amountIn) {
                            ...
                            quoteSlippage = await swapHelper.quoteSlippageExactInputSingle.staticCall({
                                tokenIn: PoolToken0,
                                tokenOut: PoolToken1,
                                fee: FeeAmount.MEDIUM,
                                amountIn: maxAmountToPay + 1n,
                        ...
                        const slippageTolerance = Number(SLIPPAGE_TOLERANCE) / Number(ONE_HUNDRED_PERCENT)
                        ...
                        const priceLimitDiff = Math.abs(priceBefore - priceLimit) / priceBefore;
                        ...
                        const priceAfter = sqrtPriceX96ToPrice(simulateResult.sqrtPriceX96After);
            ...
            await fc.assert(
                fc.asyncProperty(fc.integer({ min: 1, max: 10000 }), async (amount) => {
                    ...
                    try {
                        ...
                        const sqrtPriceLimitX96 = await getSqrtPriceLimitX96(pool, false, SLIPPAGE_TOLERANCE);
        ...
        it("should simulateSwap when zeroForOne, exactOut", async () => {
            const poolToken1Reserve = await pool.token1().then((token1: any) => {
            ...
            await fc.assert(
                fc.asyncProperty(fc.integer({ min: 1, max: 10_000 }), async (amount) => {
                    ...
                    try {
                        ...
                        const sqrtPriceLimitX96 = await getSqrtPriceLimitX96(pool, zeroForOne, SLIPPAGE_TOLERANCE);
                        ...
                        simulateResult.sqrtPriceX96After.should.be.eq(quote.sqrtPriceX96After);
                        simulateResult.sqrtPriceLimitX96.should.be.eq(sqrtPriceLimitX96);
                        if (zeroForOne) simulateResult.sqrtPriceX96After.should.be.gte(sqrtPriceLimitX96);
                        else simulateResult.sqrtPriceX96After.should.be.lte(sqrtPriceLimitX96);
                        ...
                        const user2WETHBalanceBefore = await ERC20Token.balanceOf(user2.address);
                        ...
                        user1BalanceAfter.should.be.eq(user1BalanceBefore - simulateResult.amountToPay - txFee);

                        const user2WETHBalanceAfter = await ERC20Token.balanceOf(user2.address);
                        user2WETHBalanceAfter.should.be.eq(user2WETHBalanceBefore + simulateResult.amountReceived);
                    } catch (err) {
                        console.error("❌ Failed with input:", amount, err);
                        throw err;
                    } finally {
    ...
    describe("LpSwap", () => {
        ...
        it("should quoteLpSwap", async () => {
            const maxSwapQuote = await swapHelper.quoteSlippageExactInputSingle.staticCall({
                tokenIn: VANA,
                tokenOut: ERC20Token,
                fee: FeeAmount.MEDIUM,
            ...
            await fc.assert(
                fc.asyncProperty(fc.bigInt({ min: 1n, max: 2n * maxSwapQuote.amountToPay }), async (amountIn) => {
                    const snapshotId = await network.provider.send("evm_snapshot");
                    ...
                    try {
                        ...
                        await fc.assert(
                            fc.asyncProperty(fc.bigInt({ min: 1n, max: amountIn }), async (amountSwap) => {

                                counter2++;
                                process.stdout.write(`\rquoteLpSwap (${counter1} / ${numRuns1}) ${counter2} / ${numRuns1 * numRuns2}`);

                                try {
                                    const swapQuote = await swapHelper.quoteSlippageExactInputSingle.staticCall({
                                        tokenIn: VANA,
                                        tokenOut: ERC20Token,
                                        fee: FeeAmount.MEDIUM,
                                        amountIn: amountSwap,
                                    ...
                                    BigInt(liquidity.toString()).should.be.lte(lpQuote.liquidityDelta);
                                } catch (err) {
        ...
        it("should lpSwap (fuzzing)", async () => {
            ...
            const txReceipt = await getReceipt(tx);
            (await ERC20Token.balanceOf(user1.address)).should.be.eq(wethBalanceBefore + quote.amountOut);
            (await ethers.provider.getBalance(user1.address)).should.be.eq(balanceBefore - amountIn - txReceipt.fee);
            ...
            await WVANA.connect(user1).approve(positionManager.target, amountMintInVANA);
            await ERC20Token.connect(user1).approve(positionManager.target, amountMintInToken);
                ...
                fc.asyncProperty(fc.bigInt({ min: 1n, max: parseEther(1_000_000) }), async (amountIn) => {
                    ...
                    try {
                        ...
                        if (lpQuote.liquidityDelta > 0) {
                            ...
                            tx.should.emit(positionManager, "IncreaseLiquidity")
                                .withArgs(
                                    lpTokenId,
        ...
        it("should lpSwap (single input)", async () => {
            ...
            const txReceipt = await getReceipt(tx);
            (await ERC20Token.balanceOf(user1.address)).should.be.eq(tokenBalanceBefore + quote.amountOut);
            (await ethers.provider.getBalance(user1.address)).should.be.eq(balanceBefore - amountIn - txReceipt.fee);
            ...
            await WVANA.connect(user1).deposit({ value: amountMintInVANA });
            (await WVANA.balanceOf(user1.address)).should.be.eq(wvanaBalanceBefore + amountMintInVANA);
        ...
        it("should splitRewardSwap with existing position lpTokenId (moksha)", async () => {
            ...
            const poolAddress = await factory.getPool(WVANAAddress, dlpTokenAddress, FeeAmount.MEDIUM)
            ...
            await fc.assert(
                fc.asyncProperty(fc.bigInt({ min: parseEther(1), max: parseEther(50_000) }), async (amountIn) => {
                    ...
                    counter++;
                    process.stdout.write(`\rsplitRewardSwap (moksha) ${counter} / ${numRuns}`);
                    ...
                    try {
                        ...
                        const dlpTokenBalanceBefore = await dlpToken.balanceOf(dlp.address);
                        ...
                        const treasuryVanaBalanceBefore = await ethers.provider.getBalance(treasury.address);
                        const treasuryTokenBalanceBefore = await dlpToken.balanceOf(treasury.address);

                        const poolVanaBalanceBefore = await WVANA.balanceOf(pool.target);
                        const poolTokenBalanceBefore = await dlpToken.balanceOf(pool.target);
                        ...
                        const poolTokenBalanceAfter = await dlpToken.balanceOf(pool.target);
                        poolTokenBalanceAfter.should.be.eq(poolTokenBalanceBefore - quote.tokenRewardAmount - quote.spareToken);

                        const dlpTokenBalanceAfter = await dlpToken.balanceOf(dlp.address);
                        dlpTokenBalanceAfter.should.be.eq(dlpTokenBalanceBefore + quote.tokenRewardAmount);

                        const treasuryVanaBalanceAfter = await ethers.provider.getBalance(treasury.address);
                        treasuryVanaBalanceAfter.should.be.eq(treasuryVanaBalanceBefore + quote.spareVana);

                        const treasuryTokenBalanceAfter = await dlpToken.balanceOf(treasury.address);
                        treasuryTokenBalanceAfter.should.be.eq(treasuryTokenBalanceBefore + quote.spareToken);
